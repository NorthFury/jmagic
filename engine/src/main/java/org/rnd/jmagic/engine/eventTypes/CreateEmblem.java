...
{	public static final EventType INSTANCE = new CreateEmblem();
	...
	public boolean perform(Game game, Event event, Map<Parameter, MagicSet> parameters)
	{
		...
		Set<Class<? extends Keyword>> keywords = null;
		Set<Class<? extends NonStaticAbility>> nonStaticAbilities = null;
		Set<Class<? extends StaticAbility>> staticAbilities = null;
		...
		if(null != keywords)
			for(Class<? extends Keyword> c: keywords)
				e.addAbility(Constructor.construct(c, constructorTypes, constructorArguments));
